...
If you use Kikoe with WaniKani, Kikoe needs a WaniKani API token (a read-only token is sufficient). You can paste one manually on the extension's options page, or click **Find token** there to open WaniKani's Personal Access Tokens page and ask Kikoe to read a visible token from that page while you are signed in.
...
- The Personal Access Tokens page is opened only when you click **Find token**, only on WaniKani (`www.wanikani.com`) in your browser session, so Kikoe can read a token that WaniKani already shows to you.
